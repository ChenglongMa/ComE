...
def __parse_adjacencylist_unchecked__(f):
    ...
    for line in f:
        if line and not line.startswith('#'):
            adjlist.extend([[int(x) for x in line.strip().split()]])
    return adjlist
...
def write_walks_to_disk(G, filebase, num_paths, path_length, alpha=0, rand=random.Random(0), num_workers=cpu_count()):
    ...
    :param path_length: length of each walk
...
def build_deepwalk_corpus_iter(G: nx.Graph, num_paths, path_length, alpha=0, rand=random.Random(0)):
    nodes = list(G.nodes())
    for cnt in range(num_paths):
        rand.shuffle(nodes)
        for node in nodes:
